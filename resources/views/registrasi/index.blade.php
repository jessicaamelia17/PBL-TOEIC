...
@section('content')
    <div class="max-w-xl mx-auto mt-10 bg-white rounded-lg shadow-lg p-8 animate-fadeIn">
        <h2 class="text-3xl font-bold text-center text-blue-700 mb-6">@lang('users.toeic_registration_form')</h2>
        @if ($pendaftaran)
            <div class="p-6 bg-green-50 border border-green-200 rounded text-green-800 text-center">
                <h3 class="text-xl font-bold mb-2">@lang('users.registered_done')</h3>
                <p>
                    @lang('users.exam_date_notif')
                    <b>
        ...
        @else
            ...
            <!-- Form Registrasi -->
            <form id="registrasiForm" action="{{ route('mahasiswa.registrasi.store') }}" method="POST"
                ...
                <div>
                    ...
                    <small class="text-gray-500">@lang('users.format')</small>
                    <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">@lang('users.photo_pass')</label>
                    <input type="file" name="Pas_Foto" required class="w-full px-3 py-2 border rounded" />
                    <small class="text-gray-500">@lang('users.format')</small>
                    <div>
                        <img id="previewFoto" src="#" alt="Preview Pas Foto"
                            class="mt-2 rounded shadow max-h-40 hidden" />
                    </div>
                </div>
