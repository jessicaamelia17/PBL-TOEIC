    ...
    <div class="container space-y-6">
        ...
        @php
            ...
            switch ($status) {
                case 'menunggu':
                    $ikon =
                        '<svg class="w-5 h-5 text-yellow-500 inline-block mr-2" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a8 8 0 100 16 8 8 0 000-16zM9 8h2v5H9V8zm0 6h2v2H9v-2z"/></svg>';
                    $warna = 'text-yellow-800';
                    $box = 'bg-yellow-50 border border-yellow-300';
                    break;
                case 'disetujui':
                    $ikon =
                        '<svg class="w-5 h-5 text-green-500 inline-block mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L8.414 15l-4.121-4.121a1 1 0 011.414-1.414L8.414 12.172l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>';
                    $warna = 'text-green-800';
                    $box = 'bg-green-50 border border-green-300';
                    break;
                case 'ditolak':
                    $ikon =
                        '<svg class="w-5 h-5 text-red-500 inline-block mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7.707 7.707a1 1 0 10-1.414-1.414L10 10.586l3.707-3.707a1 1 0 00-1.414-1.414L10 8.172l-2.293-2.293z" clip-rule="evenodd"/></svg>';
                    $warna = 'text-red-800';
        ...
        <div class="bg-white p-6 rounded shadow border">
            ...
            <div class="mt-6 {{ $box }} px-4 py-3 rounded flex items-center">
                ...
                <div class="{{ $warna }} font-semibold text-sm">
                    @lang('users.verification_status'): {{ $status ? __('users.status_' . $status) : '-' }}
                </div>
            </div>
            ...
            @if ($pengajuan)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-sm">
                    <p><strong>@lang('users.submission_date'):</strong>
                        {{ \Carbon\Carbon::parse($pengajuan->tanggal_pengajuan)->translatedFormat('d F Y') }}</p>
        ...
        @if ($pengajuan && $pengajuan->status_verifikasi != 'ditolak')
            <div class="bg-white p-6 rounded shadow border">
                <h3 class="text-xl font-bold text-gray-700 mb-4 border-b pb-2">@lang('users.preview_certificate')</h3>
                @if ($pengajuan->status_verifikasi == 'disetujui')
                    <a href="{{ route('mahasiswa.surat.preview', $pengajuan->id_surat) }}" target="_blank"
                        class="inline-flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition mb-4">
                        <i class="fa fa-file-pdf mr-2"></i> @lang('users.download_certificate_letter')
                    </a>
                @endif
                <p class="font-bold text-sm text-gray-600 mb-2">@lang('users.certificate2x'):</p>
                <embed src="{{ asset('storage/' . $pengajuan->file_sertifikat) }}" type="application/pdf" width="100%"
                    height="500px" class="border rounded" />
            </div>
        @endif
        ...
        @if ($pengajuan && $pengajuan->status_verifikasi == 'ditolak')
            <div class="bg-red-50 p-6 rounded shadow border border-red-300">
                <h3 class="text-xl font-semibold text-red-700 mb-2">@lang('users.app_rejection')</h3>
                <p class="mb-4 text-sm text-red-600"><strong>@lang('users.note'):</strong> {{ $pengajuan->catatan ?? '-' }}</p>

                <form action="{{ route('mahasiswa.surat.uploadUlang') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <label for="file_sertifikat" class="block font-semibold mb-1">@lang('users.reupload'):</label>
                    <input type="file" name="file_sertifikat" accept="application/pdf"
                        class="block w-full border rounded px-3 py-2 mt-1" required>
        ...
        @if (!$pengajuan)
            @if (!$sertifikat)
                <div class="bg-gray-50 p-6 rounded shadow border">
                    ...
                    <form action="{{ route('mahasiswa.surat.uploadSertifikat') }}" method="POST"
            ...
            @else
                <div class="bg-white p-6 rounded shadow border">
                    ...
                    <p><strong>@lang('users.view_certificate'):</strong>
                        <a href="{{ asset('storage/' . session('sertifikat_uploaded')) }}" class="text-blue-600 underline"
                            target="_blank">@lang('users.click_here')</a>
                    </p>

                    <form action="{{ route('mahasiswa.surat.hapusSertifikat') }}" method="POST" class="inline-block mt-3">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-600 underline text-sm hover:text-red-800 transition"
                            onclick="return confirm('Hapus sertifikat yang sudah diupload?')">
